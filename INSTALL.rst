...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/nederlandse_spoorwegen.git
       $ cd nederlandse_spoorwegen
...
``src/nederlandse_spoorwegen/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test nederlandse_spoorwegen
...
* ``DB_NAME``: name of the database for the project. Defaults to ``nederlandse_spoorwegen``.
* ``DB_USER``: username to connect to the database with. Defaults to ``nederlandse_spoorwegen``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``nederlandse_spoorwegen``.
...
1. Clone or download the code from `Github`_ in a folder like
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/nederlandse_spoorwegen.git
       Cloning into 'nederlandse_spoorwegen'...
       ...
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting nederlandse_spoorwegen_db_1 ... done
       Starting nederlandse_spoorwegen_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f nederlandse_spoorwegen_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it nederlandse_spoorwegen_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it nederlandse_spoorwegen_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/nederlandse_spoorwegen/
...
environment variables. See ``src/nederlandse_spoorwegen/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t nederlandse_spoorwegen
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
        -e DATABASE_HOST=... \
        --name nederlandse_spoorwegen \
        nederlandse_spoorwegen

    $ docker exec -it nederlandse_spoorwegen /app/src/manage.py createsuperuser
...
``src/nederlandse_spoorwegen/conf``.
